...
public class SmiteLightningEffect extends AbstractGameEffect {
    ...
    public SmiteLightningEffect(float sX, float sY, float dX, float dY) {
        if (img == null) {
            img = new TextureAtlas.AtlasRegion(ImageMaster.loadImage(DragonknightMod.LIGHTNINGBOLT),0,0,ImageMaster.loadImage(DragonknightMod.LIGHTNINGBOLT).getWidth(),ImageMaster.loadImage(DragonknightMod.LIGHTNINGBOLT).getHeight());
        }
