...
public class TransfigureVFX extends AbstractGameEffect {
    private HashMap<AbstractCard, AbstractCard> transmutedPairs;
    private TransfigureAction action;
    ...
    private static TextureRegion mask = new TextureRegion(new Texture("theDragonknightResources/images/VFX/TransmuteMask.png"), 512, 1024);
    private static TextureRegion line = new TextureRegion(new Texture("theDragonknightResources/images/VFX/TransmuteLine.png"), 512, 1024);
    private FrameBuffer fb1;
    private FrameBuffer fb2;
    private FrameBuffer fb3;
    private float particleTimer;

    public TransfigureVFX(HashMap<AbstractCard, AbstractCard> transmutedPairs, TransfigureAction action, float duration) {
        this.transmutedPairs = transmutedPairs;
        this.action = action;
        this.duration = duration;
        targetGroup = CardGroup.CardGroupType.DISCARD_PILE;
            ...
            while (particleTimer >= DURATION_PER_PARTICLE) {
                ...
                for (AbstractCard card : transmutedPairs.keySet()) {
                    ...
                    if (x < card.hb.x + card.hb.height) {
                        ...
                        AbstractDungeon.topLevelEffectsQueue.add(new DragonknightParticle(x, y, rotation, scale));
                    }
                }
            }
        } else {
