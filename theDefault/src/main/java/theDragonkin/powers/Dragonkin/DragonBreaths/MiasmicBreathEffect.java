...
public class MiasmicBreathEffect extends AbstractDragonBreathPower{
    ...
    public MiasmicBreathEffect (int Markamt , AbstractCard Source){
        ...
        ID = POWER_ID;
        CallingCard = Source;
        amount4 =Markamt+((BreathCount -1));
    }

    @Override
    public void onBreath(){
        for (AbstractMonster m : AbstractDungeon.getCurrRoom().monsters.monsters){
            addToBot(new ApplyPowerAction(m,owner,new AcidMarkPower(m,owner,amount4),
                    amount4+(BreathCount), AbstractGameAction.AttackEffect.POISON));
        }
