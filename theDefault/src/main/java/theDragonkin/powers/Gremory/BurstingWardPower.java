...
public class BurstingWardPower extends AbstractPower {
    ...
    public void atEndOfTurn(final boolean isPlayer){
        for (AbstractMonster m : AbstractDungeon.getCurrRoom().monsters.monsters) {
            if (m.getIntentBaseDmg() >= 0 && !m.isDeadOrEscaped()) {
                if (m.hasPower(ResistancePower.POWER_ID)){
                    addToBot(new DamageAction(m,new DamageInfo(AbstractDungeon.player,this.amount + m.getPower(ResistancePower.POWER_ID).amount, DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.FIRE));
                } else {
                    addToBot(new DamageAction(m,new DamageInfo(AbstractDungeon.player,this.amount, DamageInfo.DamageType.THORNS), AbstractGameAction.AttackEffect.FIRE));
                }
