...
public class Krah extends CustomOrb implements OnChannelOrb {
    public static final String ORB_ID = DragonkinMod.makeID("Krah");
    ...
    public void onEvoke(){
        ...
        if (m != null) {
            ...
            AbstractDungeon.actionManager.addToBottom(new VFXAction(new BlizzardEffect(4,false)));
            for (AbstractMonster M : AbstractDungeon.getCurrRoom().monsters.monsters){
                if (!M.isDeadOrEscaped()){
                    AbstractDungeon.actionManager.addToBottom(new DamageAction(M,new DamageInfo(AbstractDungeon.player,applyLockOn(M,evokeAmount))));
                }
            }
        }
    }
    ...
    public void onEndOfTurn() {
        ...
        for (AbstractOrb orb : AbstractDungeon.player.orbs){
            if (orb instanceof Fo || orb instanceof Diin){
                MemberCount = 1;
            }
        }
        if (MemberCount > 0){
